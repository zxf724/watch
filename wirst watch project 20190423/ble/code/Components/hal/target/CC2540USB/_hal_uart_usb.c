...
__no_init static uint8 halUartRxQ[HAL_UART_USB_FIFO_SIZE];
__no_init static uint8 halUartTxQ[HAL_UART_USB_FIFO_SIZE];

static uint16 halUartRxH, halUartRxT;
static uint16 halUartTxH, halUartTxT;
...
uint16 HalUARTRx(uint8 *buf, uint16 max);
uint16 HalUARTRx(uint8 *buf, uint16 max)
{
  uint16 cnt = 0;
  ...
  while ((halUartRxH != halUartRxT) && (cnt < max))
  {
    *buf++ = halUartRxQ[halUartRxH++];
    halUartRxH = (halUartRxH % sizeof(halUartRxQ)); //loopback
    cnt++;
  }
...
uint16 HalUARTTx(uint8 *buf, uint16 cnt);
uint16 HalUARTTx(uint8 *buf, uint16 cnt)
{
  while (cnt--)
  {
    halUartTxQ[halUartTxT++] = *buf++;
    halUartTxT = (halUartTxT % sizeof(halUartTxQ)); //loopback
  }
...
static void halUartPollRx(void)
{
  ...
  if (USBFW_OUT_ENDPOINT_DISARMED())
  {
    ...
    while (cnt--)
    {
      ...
      halUartRxT = (halUartRxT % sizeof(halUartRxQ)); //loopback
    }
  ...
  if (USBFW_IN_ENDPOINT_DISARMED())
  {
    ...
    else
    {
      ...
      do
      {
        ...
        halUartTxH = (halUartTxH % sizeof(halUartTxQ)); //backloop
