...
/* LED select */
#define LED_UART_TX   HAL_LED_2
#define LED_UART_RX   HAL_LED_1
#define LED_BLE_LINK  HAL_LED_3
#define LED_BLE_PAIR  HAL_LED_4

/* MCU Reset controll */
#define RESET_CTRL_ENABLE()  st(RESET_CTRL_SBIT = RESET_CTRL_POLARITY (0);)
#define RESET_CTRL_DISABLE() st(RESET_CTRL_SBIT = RESET_CTRL_POLARITY (1);)

#define CMD_LEN_MIN  7
/*********************************************************************
 * TYPEDEFS
 */
typedef struct
 ...
 * GLOBAL VARIABLES
 */
uint8 g_OffDebugPrint = 1; //off usb port print the dubeg info
uint8 g_UsbUpgradeMode = FALSE; //default off usb upgrade
uint8 g_McuResetCount = 0;
uint8 g_McuReset = FALSE;
uint8 g_cmd_packet[10];
uint16 g_cmd_packet_len;


static unsigned int  UsbmsgLength   = 0;
static unsigned char *UsbmsgBuffer = NULL;
/*********************************************************************
 * EXTERNAL VARIABLES
 */
...
uint8 osal_get_event( uint8 task_id, uint16 event_flag );
static void Hci_ext_Usb2Uart();
static void Hci_ext_Uart2Usb();
...
void Uart_Init(uint8 baud)
{
  PERCFG = 0x00;
  P0SEL = 0x0c; //P0_2,P0_3 as uart
  P2DIR &= ~0xC0;
  U0CSR |= 0x80;
  
  if(baud == HAL_UART_BR_38400)
  {
    U0GCR = 10;
    U0BAUD = 59; //baud rate 38400
  }
  else if(baud == HAL_UART_BR_115200)
  {
    U0GCR = 11;
    U0BAUD = 216; //baud rate 115200
  }
  else
  {
    U0GCR = 11;
...
void UartSendString(uint8 *Data, int len)
{
  int i;
  for(i=0; i<len; i++)
  {
...
static unsigned char msgParseState;
...
void Usbsendchar2Uart(uint8 data)
{
  UartSendString(&data,1);//usb to uart
}

void sendchar(uint8 data)
{
  NPI_WriteTransport(&data, 1); //uart to usb
}
...
__interrupt void UART0_ISR(void)
{
  ...
  //uart to usb
  if(g_UsbUpgradeMode == FALSE)
  {
     NPI_WriteTransport(&c,1);
  } 
  else if(osal_get_event( hciExtApp_TaskID, SBP_UART_RX_EVT) != SUCCESS)
  {
     /*
		 * Collect received bytes to a complete message
		 */
			if( msgParseState != ST_PROCESS )
			{
                ...
                osal_start_timerEx( hciExtApp_TaskID, SBP_UART_RX_EVT, 20);
						}
						else
						{
							msgParseState	=	ST_START;
...
void HCI_EXT_App_Init( uint8 task_id )
{
  ...
  //MCU Reset Control, gpio output mode
  RESET_CTRL_DDR |= RESET_CTRL_BV;

  //disable mcu reset
  RESET_CTRL_DISABLE();

  //serial port initialization
  Uart_Init(HAL_UART_BR_38400);
...
HalLedBlink(HAL_LED_ALL, 6, 50,400);
...
uint16 HCI_EXT_App_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if( events & SBP_USB_RX_EVT)
  {
    Hci_ext_Usb2Uart();
    
    return (events ^ SBP_USB_RX_EVT);
  }
  
  if( events & SBP_UART_RESET_EVT)
  {    
    if(g_McuReset)
    {
      //release mcu reset pin
      RESET_CTRL_DISABLE();
      g_McuReset = FALSE;
      msgParseState = ST_START;
      UartSendString(g_cmd_packet,g_cmd_packet_len);
    }
    
    return (events ^ SBP_UART_RESET_EVT);
  }

  if( events & SBP_UART_RX_EVT)
  {
    Hci_ext_Uart2Usb();

    return (events ^ SBP_UART_RX_EVT);
  }  
...
void simpleBLE_SendData(uint8* buffer, uint16 sendBytes)
{
    //if( GAPROLE_CONNECTED == gapProfileState) // ÒÑÁ¬½ÓÉÏ
    if(0)
    {
    }
    else
    {
         ...
         UartSendString(buffer, sendBytes); //usb to uart
         HalLedBlink(LED_UART_TX, 1, 50, 20); //uart TX LED blink
    }
}
...
static void Hci_ext_Uart2Usb()
{
  ...
  if(msgParseState  == ST_PROCESS)
  {
    ...
    */
    ...
    NPI_WriteTransport(buf, i);

    //after leave isp program mode, uart set to default status   
    p = msgBuffer;
    if(*p == CMD_LEAVE_PROGMODE_ISP)
    {
       g_UsbUpgradeMode = FALSE;
       g_McuResetCount = 0;
       Uart_Init(HAL_UART_BR_38400);
       HalLedBlink(LED_UART_TX | LED_UART_RX , 3, 50, 1000);
    }
    else
    {
      HalLedBlink(LED_UART_RX, 1, 50, 20); //uart RX LED blink
    }
...
static void Hci_ext_Usb2Uart()
{
  uint8 *buffer = UsbmsgBuffer;
  uint8 err = 0;
  
  if(buffer)
  {     
     if(UsbmsgLength >= CMD_LEN_MIN)
     {
        /* uint8 seq = buffer[1]; */
        uint16 pkt_len = buffer[2];
        pkt_len <<= 8;
        pkt_len |= buffer[3];
        pkt_len += 6;
        ...
        if(cksum == buffer[pkt_len - 1])
        {
          if(cmd == CMD_SIGN_ON)
          {  
             g_McuResetCount++;

             if(g_McuResetCount < 6)
             {
               //make reset mcu singal
               RESET_CTRL_ENABLE();                           
              
               osal_memcpy(g_cmd_packet, buffer, pkt_len);
               
               g_cmd_packet_len = pkt_len;

               osal_start_timerEx( hciExtApp_TaskID, SBP_UART_RESET_EVT, 500);
               
               g_McuReset = TRUE;

               g_UsbUpgradeMode = TRUE;
               Uart_Init(HAL_UART_BR_115200);
               
               RESET_CTRL_DISABLE();
             }
             else
             {
               //timeout, release mcu reset pin
               RESET_CTRL_DISABLE();

               g_McuResetCount = 0;

               //timeout, uart set to default status
               g_UsbUpgradeMode = FALSE;
               Uart_Init(HAL_UART_BR_38400);
             }
           }
           ...
           if(!g_McuReset)
           {
              simpleBLE_SendData(buffer,pkt_len);
           }
        }
        else
        {
      ...
      if(err)
      {
        if(g_UsbUpgradeMode == FALSE)
        {
          simpleBLE_SendData(buffer,UsbmsgLength);
        }
      }
...
uint8 osal_get_event( uint8 task_id, uint16 event_flag )
{
  if ( task_id < tasksCnt )
  {
    ...
    if(tasksEvents[task_id] & event_flag)
      result = 1;
...
static void NpiSerialCallback( uint8 port, uint8 events )
{
    (void)port;
    uint16 numBytes;
    uint16 readMaxBytes = 272;
    
    if (events & (HAL_UART_RX_TIMEOUT | HAL_UART_RX_ABOUT_FULL))
    {
        ...
        if(osal_get_event(hciExtApp_TaskID, SBP_USB_RX_EVT) == SUCCESS)
        {
            //usb data is being parsed and reception is not allowed
            return;
        }
        ...
        if(UsbmsgLength == 0)
        {
            ...
            if(UsbmsgLength < readMaxBytes)
            {
              UsbmsgBuffer = osal_mem_alloc(readMaxBytes);

              if(UsbmsgBuffer)
              {
                NPI_ReadTransport(UsbmsgBuffer,numBytes);
                UsbmsgLength += numBytes;
                //start timeout
                osal_start_timerEx( hciExtApp_TaskID, SBP_USB_RX_EVT, 20);
              }
            }
        }
        else if(UsbmsgBuffer)
        {
            if(UsbmsgLength < readMaxBytes)
            {
              NPI_ReadTransport(&UsbmsgBuffer[UsbmsgLength],numBytes);
              UsbmsgLength += numBytes;
            }
            else
            {
               osal_stop_timerEx(hciExtApp_TaskID, SBP_USB_RX_EVT);
               osal_set_event(hciExtApp_TaskID, SBP_USB_RX_EVT);
            }
        } 
    }
}
